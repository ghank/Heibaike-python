...
class Window(wx.Window):
    ...
    def slideUpDown(self,up):
        ...
        for row in range(numRows):
            for col in range(numCols): oldData[row][col] = self.data[row][col]
    ...
    def isGameOver(self):
        numRows = len(self.data)
        numCols = len(self.data[0])
        copyData = [[0 for i in range(numCols)] for j in range(numRows)] #[[0]*numCols]*numRows X error
        for row in range(numRows):
            for col in range(numCols):
    ...
    def doMove(self,move,score):
        if move:
            ...
            if self.isGameOver():
                ...
                if wx.MessageBox(cn("游戏结束，是否重新开始？"),cn("哈哈"),wx.YES_NO|wx.ICON_INFORMATION)==wx.YES:
    ...
    def drawLabel(self,dc):
        ...
        dc.DrawText(u"HOW TO PLAY: Use your arrow keys to move the tiles. When two\ntiles with the same number touch, they merge into one!",15,639)
...
class App(wx.App):
    ...
    def OnInit(self):
        self.frame = Frame(u"2048 v1.0.0 by 律酱大爱")
