...
class MetaBase(type):
    def __new__(mcl, name, bases, atts):
        print('MetaBase.__new__\n')
        return super(MetaBase, mcl).__new__(mcl, name, bases, atts)

    def __init__(cls, name, bases, atts):
        print('MetaBase.__init__\n')
        super(MetaBase, cls).__init__(name, bases, atts)
...
class MetaNewVSInit(MetaBase):
    def __new__(mcl, name, bases, atts):
        ...
        bases += (Tag1,)
        atts['baz'] = 42
        return super(MetaNewVSInit, mcl).__new__(mcl, name, bases, atts)

    def __init__(cls, name, bases, atts):
        # First argument is the class being initialized
        print('MetaNewVSInit.__init__')
        for x in (cls, name, bases, atts): pprint(x)
        print('')
        if 'bar' in atts: atts.pop('bar') # No effect
        name += '_y' # No effect
        bases += (Tag2,) # No effect
        atts['pi'] = 3.14159 # No effect
        super(MetaNewVSInit, cls).__init__(name, bases, atts)
