...
class Window(wx.Window):
    ...
    def __init__(self, parent):
        ...
        16:(245, 149, 99),32:(246, 124, 95),64:(246, 94, 59),128:(237, 207, 114),256:(237, 207, 114),
        512:(237, 207, 114),1024:(237, 207, 114),2048:(237, 207, 114),4096:(237, 207, 114),8192:(237, 207, 114),
        16384:(237, 207, 114),32768:(237, 207, 114),65536:(237, 207, 114),131072:(237, 207, 114),262144:(237, 207, 114),
        524288:(237, 207, 114),1048576:(237, 207, 114),2097152:(237, 207, 114),4194304:(237, 207, 114),8388608:(237, 207, 114),
        16777216:(237, 207, 114),33554432:(237, 207, 114),67108864:(237, 207, 114),134217728:(237, 207, 114),268435456:(237, 207, 114),
        536870912:(237, 207, 114),1073741824:(237, 207, 114),2147483648:(237, 207, 114),4294967296:(237, 207, 114),8589934592:(237, 207, 114),
        17179869184:(237, 207, 114),34359738368:(237, 207, 114),68719476736:(237, 207, 114),137438953472:(237, 207, 114)}
        self.initGame()
        self.initBuffer()
    ...
    def loadScore(self):
        if os.path.exists("bestscore.ini"):
            fp = open('bestscore.ini')
            self.bstScore = fp.read()
            fp.close()

    #保存分数
    def saveScore(self):
        fp = open('bestscore.ini', 'w')
        fp.write(str(self.bstScore))
        fp.close()

    #1、字体颜色 2、获取分数 3、初始化数据区
    def initGame(self):
    ...
    def onKeyDown(self,event):
        ...
        if keyCode==wx.WXK_UP:
            tup = self.slideUpDown(True)
            self.doMove(*tup)
            while (len(self.history)>self.maxlen):
                del self.history[0]
            self.history.append(copy.deepcopy(tup))
            self.cur = len(self.history)-1
        elif keyCode==wx.WXK_DOWN:
            tup = self.slideUpDown(False)
            self.doMove(*tup)
            while (len(self.history)>self.maxlen):
                del self.history[0]
            self.history.append(copy.deepcopy(tup))
            ...
            self.cur -= 1
            if self.cur>=0:
                self.doMove(*self.history[self.cur])
            else:
                self.cur = -1
...
class App(wx.App):
    ...
    def OnInit(self):
        ...
        self.frame.SetClientSize((500,720))
        self.frame.Center()
        self.frame.Show()
        return True
