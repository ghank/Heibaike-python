...
# 步骤：
# 1、初始化一个4*4的二维数组，在在这个数组随机选两个位置，放置2或4的数值；
# 2、绘制一个画布，并将数据在画布上展示；
# 3、根据键盘操作情况，更新画布
    ...
    def slideUpDown(self,up):
        ...
        for col in range(numCols):
            ...
            for i in range(numRows-len(cvl)):
                if up: cvl.append(0)#尾部追加0
                else: cvl.insert(0,0)#头部追加0
            for row in range(numRows): self.data[row][col] = cvl[row]#更新此列的数据
        return oldData!=self.data,score#返回增加的分数
